...
import { getContinuedGames } from "@/lib/continuedGames";
...
export default function Home() {
  ...
  // Featured shelf order, never an empty crash: fall back to the first entries.
  const featured = gameRegistry.filter((game) => game.featured);
  const showcaseA = featured[0] ?? gameRegistry[0];
  const showcaseB = featured[1] ?? gameRegistry[1];
  return <div>
    ...
    <section className="games-feature" aria-labelledby="home-games-title"><div className="site-frame games-feature-inner"><div className="games-feature-copy"><p className="eyebrow">{t("home.stepPlay")}</p><h2 id="home-games-title">{t("home.gamesTitle")}</h2><p>{t("home.gamesCopy")}</p><Link href="/games" className="dark-cta"><Gamepad2 className="size-4" aria-hidden="true" />{t("home.games")}<ArrowRight className="size-4" aria-hidden="true" /></Link></div><div className="game-showcase"><WorkshopPlate variant="games" />{showcaseA && showcaseB && <><Link href={`/games/${showcaseA.slug}`} className="floating-game-card"><span><Layers3 className="size-4" /></span><strong>{showcaseA.name}</strong><small>{language === "bn" ? showcaseA.genreBn : showcaseA.genre}</small></Link><Link href={`/games/${showcaseB.slug}`} className="floating-game-card second"><span><Sparkles className="size-4" /></span><strong>{showcaseB.name}</strong><small>{language === "bn" ? showcaseB.genreBn : showcaseB.genre}</small></Link></>}</div></div></section>
    {continued.length > 0 && <section className="site-frame section-space" aria-labelledby="home-continue-title"><div className="section-heading"><div><h2 id="home-continue-title">{t("home.continue")}</h2><p>{t("home.continueCopy")}</p></div><Link href="/games" className="text-link">{t("home.viewAll")}<ArrowRight className="size-4" aria-hidden="true" /></Link></div><div className="game-grid">{continued.map(({ game, save }) => { const Icon = game.icon; return <Link key={game.slug} href={`/games/${game.slug}`} className="game-card"><div className="game-card-top"><span className="game-icon" aria-hidden="true"><Icon className="size-5" /></span><span>{language === "bn" ? game.genreBn : game.genre}</span></div><div><h2>{game.name}</h2><p>{game.description[language]}</p><span className="game-play-label">{t("game.best")}: {save.highScore} <span aria-hidden="true">→</span></span></div></Link>; })}</div></section>}
